...
char string_pool[2][16] = {"    COOKING  ", "      SIM"};
char string_pool_started[2][17] = {"MONEY:          ", "C:              "};
...
TASK(LCD1)
{
    ...
    SetRelAlarm(ALARM_LCD, 1, 500);
    while(1)
    {
        WaitEvent(LCD1_EVENT);
        ClearEvent(LCD1_EVENT);
        Lcd_refresh();
    }
    TerminateTask();
}
